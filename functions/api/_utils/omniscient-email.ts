import type { AutomationBrowserContext, AutomationPage } from './omniscient-browser';
...
function domainMatchesWebsite(email: string, website: string | null | undefined): boolean {
    const businessHost = normalizeWebsiteHost(website);
...
export function validatePhone(phone: string | null | undefined): {
    ...
    if (digits.length < 10) {
        return { confidence: 0.1, flags: ['too_short'] };
    }

    if (digits.length > 11) {
        return { confidence: 0.2, flags: ['too_long'] };
    }
    ...
    if (new Set(digits.slice(-7)).size === 1) {
        confidence = 0.1;
...
export function resolvePublicBusinessEmail(input: {
    ...
    for (const [email, occurrences] of occurrencesByEmail.entries()) {
        ...
        for (const occurrence of occurrences) {
            ...
            if (!seenSources.has(occurrence.sourceLabel)) {
                seenSources.add(occurrence.sourceLabel);
            }
        }

        score += Math.min((seenSources.size - 1) * 8, 16);
    ...
    return {
        ...
        reason: winner ? `${winner.type} email chosen from ${winner.occurrences[0]?.sourceLabel || 'public source'}` : 'No vetted public email found',
        candidates,
    };
}

async function capturePageSnapshot(page: AutomationPage): Promise<{ text: string; links: ResolvedLink[] }> {
...
function buildDiscoverySection(label: string, snapshot: { text: string; links: ResolvedLink[] }): string {
    ...
    try {
        ...
        for (const link of contactLinks) {
            ...
            try {
                ...
                // keep going with partial discovery
            } finally {
                await subPage.close();
            }
        }
...
export function pickBestSocialLink(pages: EmailDiscoveryPage[]): string {
    ...
    const candidates = pages.flatMap((page) => page.links).map((link) => link.href).filter(Boolean);
