      ...
      <div className="relative overflow-hidden rounded-full border border-white/10 bg-[#111]/40 backdrop-blur-md">
        <div className="animate-logo-cloud flex w-max items-center gap-6 px-6 py-4">
          {track.map((icon, index) => (
            <div
              key={`${icon.name}-${index}`}
              className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.02] px-5 py-2 text-[#A7B3BC] grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all duration-500 transform-gpu"
              title={icon.name}
