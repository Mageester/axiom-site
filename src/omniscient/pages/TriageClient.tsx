...
import { Skeleton } from "@omni/components/ui/skeleton";
import {
...
} from "@omni/lib/ui/triage-store";
import { addToCallList, removeFromCallList } from "@omni/lib/ui/storage";
import { TriageFilterBar } from "@omni/components/triage/triage-filter-bar";
import { TriageCard, type TriageLead } from "@omni/components/triage/triage-card";
import { ActionDock } from "@omni/components/triage/action-dock";
import { CallListDrawer, type CallListEntry } from "@omni/components/triage/call-list-drawer";
...
function TriageInner() {
    ...
    useEffect(() => {
        ...
        fetch(`/api/omniscient/leads/triage?${params}`)
            .then(r => r.json())
            .then(data => {
    ...
    const handleOpenDossier = useCallback(() => {
        if (currentLead) navigate(`/lead/${currentLead.id}`);
    }, [currentLead, navigate]);
