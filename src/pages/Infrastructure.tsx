...
const ProcessTimeline: React.FC = () => (
  <RevealBlock
    as="div"
    variant="feature"
    className="reveal-stagger mt-7 overflow-hidden rounded-[var(--radius-card)] border border-white/10 bg-[linear-gradient(180deg,rgba(16,21,31,0.98)_0%,rgba(9,12,18,0.99)_100%)] p-5 shadow-[0_14px_32px_rgba(0,0,0,0.18)] md:p-7"
  >
    <div className="relative flex flex-col gap-5 lg:flex-row lg:items-start lg:gap-4">
      <div className="absolute left-[0.35rem] top-3 bottom-8 w-[1px] bg-white/10 lg:hidden" aria-hidden="true" />
      {PROCESS_STEPS.map((step, index) => (
        <React.Fragment key={step.id}>
          ...
          >
            <span className="absolute left-0 top-[0.4rem] h-3 w-3 rounded-full border border-white/20 bg-[#0f1524] lg:hidden" aria-hidden="true" />
            <p className="font-axiomDisplay text-[clamp(1.6rem,3vw,2.2rem)] leading-none text-accent">{step.dayLabel}</p>
            <h3 className="mt-3 text-lg font-semibold text-white">{step.title}</h3>
            <p className="mt-2 text-sm md:text-[15px] leading-6 text-[#A7B3BC]">{step.timelineDescription}</p>
          </RevealBlock>
      ...
      <Layout>
        <main id="main-content" tabIndex={-1} className="mx-auto w-full max-w-[92rem] px-5 pb-16 md:px-10 md:pb-24">
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(22rem,0.7fr)] xl:items-start xl:gap-8">
              <div>
                <div className="mb-5 flex flex-col gap-2 md:mb-6">
                  ...
                  <p className="max-w-xl text-sm md:text-[15px] leading-relaxed text-slate-300">
              ...
              <article className="overflow-hidden rounded-[var(--radius-card)] border border-white/10 bg-white/[0.03] p-5 shadow-[0_18px_42px_rgba(0,0,0,0.18)] md:p-6 xl:sticky xl:top-28">
                ...
                <p className="mt-2 max-w-md text-sm md:text-[15px] leading-relaxed text-slate-300">
                ...
                <ul className="mt-5 divide-y divide-white/[0.08]">
                  {CHECKLIST_ITEMS.map((item) => (
                    <li key={item.title} className="flex items-start gap-3 py-3.5 first:pt-0 last:pb-0">
                      <span className="mt-[3px] flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/[0.03]">
                        <CheckMark />
                      </span>
                      <div>
                        <p className="text-sm md:text-[15px] font-medium leading-relaxed text-slate-200">
                          {item.title}
                        </p>
                        <p className="mt-1 text-sm md:text-[15px] leading-relaxed text-slate-300">
                          {item.detail}
                        </p>
                      </div>
                    </li>
          ...
          <section className="pt-8 md:pt-10">
            <article className="overflow-hidden rounded-[var(--radius-card)] border border-white/10 bg-[linear-gradient(180deg,rgba(16,21,31,0.96)_0%,rgba(10,13,19,0.98)_100%)] p-6 shadow-[0_18px_44px_rgba(0,0,0,0.22)] md:p-8 lg:p-10">
              <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-end">
                <div>
                  ...
                  <p className="mt-3 max-w-xl text-sm md:text-[15px] leading-relaxed text-slate-300 lg:mt-3">
