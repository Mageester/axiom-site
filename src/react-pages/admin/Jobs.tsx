    ...
    return (
        <div className="pt-32 pb-24 px-6 max-w-[1100px] mx-auto w-full">
            <div className="flex justify-between items-end mb-10 border-b border-axiom-border pb-6">
                <div>
                    <h1 className="text-3xl font-semibold text-axiom-text-main tracking-tight">System Job Queue</h1>
                    <p className="text-[13px] text-axiom-text-mute font-mono uppercase tracking-widest mt-2">{jobs.length} Recent Tasks</p>
                </div>
                <button
                    disabled={running}
                    onClick={handleRun}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-10">
                {stats.length > 0 ? stats.map((s, i) => (
                    <div key={i} className="axiom-bento p-5 bg-axiom-base/40 border border-axiom-border flex flex-col items-center">
                        ...
                        <span className="text-[10px] text-axiom-text-mute/60 font-mono tracking-widest uppercase mt-1">{s.status}</span>
                    </div>
                )) : (
                    <div className="text-axiom-text-mute font-mono text-[11px] lg:col-span-4 text-center">No active telemetry stats</div>
                )}
            </div>

            <div className="axiom-bento overflow-hidden border border-axiom-border rounded-sm">
                {loading && jobs.length === 0 ? <p className="p-6 text-axiom-text-mute font-mono text-[12px]">Connecting to queue module...</p> : (
                        ...
                        <tbody className="divide-y divide-white/5">
                            {jobs.map(job => (
                                <tr key={job.id} className="hover:bg-white/[0.02] transition-colors">
                                    <td className="p-4 font-medium text-axiom-text-main font-mono text-[11px]">{job.type}</td>
                                    <td className="p-4 text-axiom-text-mute/60 text-[11px] font-mono">{new Date(job.created_at).toLocaleString()}</td>
                                    <td className="p-4 text-axiom-text-mute/80 font-mono">{job.attempts}</td>
                                    <td className="p-4">
                                        {job.last_error ? <span className="text-[10px] font-mono text-red-500/80 break-words">{job.last_error.substring(0, 50)}...</span> : <span className="text-axiom-text-mute/40 font-mono text-[10px] italic">OK</span>}
                                    </td>
